...
impl Uri {
    // This clippy allow mirrors stdx::Url::from_file_path
    #[allow(clippy::result_unit_err)]
    pub fn to_url(&self) -> Result<stdx::Url, ()> {
        match self {
            Uri::File(path) => stdx::Url::from_file_path(path),
        }
    }
...
fn convert_url_to_uri(url: &stdx::Url) -> Result<Uri, UrlConversionErrorKind> {
    if url.scheme() == "file" {
        url.to_file_path()
            .map(|path| Uri::File(stdx::path::normalize(path).into()))
...
impl TryFrom<stdx::Url> for Uri {
    type Error = UrlConversionError;

    fn try_from(url: stdx::Url) -> Result<Self, Self::Error> {
