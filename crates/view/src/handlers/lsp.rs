...
pub enum ApplyEditErrorKind {
    DocumentChanged,
    FileNotFound,
    InvalidUrl(editor_core::uri::UrlConversionError),
...
impl From<editor_core::uri::UrlConversionError> for ApplyEditErrorKind {
    fn from(err: editor_core::uri::UrlConversionError) -> Self {
...
impl Editor {
    ...
    pub fn handle_lsp_diagnostics(
        ...
        if let Some(doc) = doc {
            ...
            let doc = doc.id();
            event::dispatch(DiagnosticsDidChange { editor: self, doc });
        }
    }
