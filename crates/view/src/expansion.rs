...
use editor_core::command_line::{ExpansionKind, Token, TokenKind, Tokenizer};
...
pub enum Variable {
    ...
    /// Nearest ancestor directory of the current working directory that contains `.git`, `.svn`, `jj` or `.mitos`
    WorkspaceDirectory,
...
fn expand_variable(editor: &Editor, variable: Variable) -> Result<Cow<'static, str>> {
    ...
    match variable {
        ...
        Variable::CursorColumn => {
            ...
            let position = editor_core::coords_at_pos(text, cursor);
        ...
        Variable::FilePathAbsolute => {
            let path = match doc.path() {
                ...
                None => stdx::env::current_working_dir(),
            }
            .to_string_lossy()
            .to_string();
            Ok(Cow::Owned(path))
        ...
        Variable::CurrentWorkingDirectory => Ok(std::borrow::Cow::Owned(
            stdx::env::current_working_dir()
                .to_string_lossy()
                .to_string(),
        )),
        Variable::WorkspaceDirectory => Ok(std::borrow::Cow::Owned(
            loader::find_workspace().0.to_string_lossy().to_string(),
        )),
