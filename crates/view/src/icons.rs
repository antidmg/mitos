...
impl Icon {
    ...
    pub fn set_glyph(&mut self, glyph: &'static str) {
        self.glyph = Some(StackString::from(glyph).unwrap());
    }
...
impl File {
    ...
    pub fn get_by_filename<P: AsRef<Path>>(&self, path: P) -> Option<Icon> {
        ...
        FILE.filename.get(name.as_ref()).copied()
    }
    ...
    pub fn get_by_extension<P: AsRef<Path>>(&self, path: P) -> Option<Icon> {
        ...
        FILE.extension.get(ext.as_ref()).copied()
    }
    ...
    pub fn get_with_style_or_default<P: AsRef<Path>>(&self, path: P, theme: &Theme) -> Icon {
        ...
        // Order matters: exact filenames take precedence over extensions.
        let style = [name.as_ref(), ext.as_ref()]
            .into_iter()
            .flatten()
...
impl Directory {
    ...
    pub fn get(&self, name: &str, is_open: bool) -> Icon {
        if is_open {
            return DIRECTORY
                ...
                .get("open")
                .copied()
                .expect("the open-directory icon is always defined");
        }
        ...
        DIRECTORY
            ...
            .copied()
            .expect("the default-directory icon is always defined")
    }
    ...
    pub fn get_with_style_or_default(
        ...
        let style = self.get_style_from_theme(theme, name).map_or_else(
            || base.patch(icon.style()),
            |style| base.patch(icon.style()).patch(style),
        );
...
mod test {
    ...
    fn kind_should_always_contain_color_icon() {
        ...
        assert_eq!(icon.glyph().as_str(), "■");
    }

    #[test]
    fn bundled_icons_cover_files_and_symbol_kinds() {
        let icons = Icons::default();
        assert!(icons.fs().file().unwrap().get("main.rs").is_some());
        assert!(icons.kind().get("function").is_some());
    }
}
