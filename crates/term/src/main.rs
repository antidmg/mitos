...
use term::args::Args;
use term::config::{Config, ConfigLoadError};
...
fn setup_logging(verbosity: u64) -> Result<()> {
    ...
    term::logging::init_file(level, &loader::log_file())?;
...
async fn main_impl() -> Result<i32> {
    ...
    loader::initialize_config_file(args.config_file.clone());
    loader::initialize_log_file(args.log_file.clone());
...
",
            ...
            loader::default_log_file().display(),
        );
        std::process::exit(0);
    }
    ...
    if args.health {
        if let Err(err) = term::health::print_health(args.health_arg) {
    ...
    if args.fetch_grammars {
        loader::grammar::fetch_grammars(args.strict)?;
        return Ok(0);
    }

    if args.build_grammars {
        loader::grammar::build_grammars(None, args.strict)?;
        return Ok(0);
    }
    ...
    if let Some(path) = &args.working_directory {
        stdx::env::set_current_working_dir(path)?;
