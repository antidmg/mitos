...
/// A wrapper around `lsp::Location` that swaps out the LSP URI for `editor_core::Uri` and adds
...
pub(crate) fn code_actions_for_range(
    ...
    impl Future<Output = Result<Option<Vec<CodeActionOrCommand>>, lsp_client::Error>>,
    LanguageServerId,
)> {
    let mut seen_language_servers = HashSet::new();
...
fn code_action_on_save_step(
    ...
    let build_request = move |editor: &mut Editor| -> Option<Job> {
        ...
        let full_range = editor_core::Range::new(0, doc.text().len_chars());
...
/// several edits.
type ResolveFuture =
    std::pin::Pin<Box<dyn Future<Output = Result<lsp::CodeAction, lsp_client::Error>> + Send>>;
enum ResolveStep {
...
fn goto_impl(editor: &mut Editor, compositor: &mut Compositor, locations: Vec<Location>) {
    let cwdir = stdx::env::current_working_dir();
...
where
    ...
    F: Future<Output = lsp_client::Result<Option<lsp::GotoDefinitionResponse>>> + 'static + Send,
{
...
pub fn rename_symbol(cx: &mut Context) {
    fn get_prefill_from_word_boundary(editor: &Editor) -> String {
        ...
        if primary_selection.len() > 1 {
            ...
            use editor_core::textobject::{textobject_word, TextObject};
...
fn compute_inlay_hints_for_view(
    ...
    let range = lsp_client::util::range_to_lsp_range(
        doc_text,
        editor_core::Range::new(first_char_in_range, last_char_in_range),
    ...
    let callback = super::make_job_callback(
        ...
        move |editor, _compositor, response: Option<Vec<lsp::InlayHint>>| {
            ...
            for hint in hints {
                let char_idx = match lsp_client::util::lsp_pos_to_pos(
                    doc_text,
                    hint.position,
                    offset_encoding,
                ) {
