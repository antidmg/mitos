...
fn dap_callback<T, F>(
    jobs: &mut Jobs,
    call: impl Future<Output = dap::Result<serde_json::Value>> + 'static + Send,
    callback: F,
) where
...
pub fn dap_start_impl(
    ...
    if !cx
        .editor
        .workspace_trust
        .query(&workspace, loader::workspace_trust::TrustQuery::Dap)
        .is_trusted()
    {
    ...
    args.insert("cwd", to_value(stdx::env::current_working_dir())?);
