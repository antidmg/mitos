...
use editor_core::Uri;
use event::{cancelable_future, register_hook, send_blocking};
...
use view::handlers::diagnostics::DiagnosticEvent;
use view::handlers::lsp::{PullAllDocumentsDiagnosticsEvent, PullDiagnosticsEvent};
...
impl event::AsyncHook for PullDiagnosticsHandler {
...
impl event::AsyncHook for PullAllDocumentsDiagnosticHandler {
        ...
        loop {
            match cancelable_future(futures.next(), &cancel).await {
                ...
                Some(Some((Err(err), DiagnosticProvider::Lsp { server_id, .. }, _))) => {
                    let parsed_cancellation_data = if let lsp_client::Error::Rpc(error) = err {
