...
╚═╝     ╚═╝╚═╝   ╚═╝    ╚═════╝ ╚══════╝"#;
    ...
    pub fn doc_diagnostics_highlights_into(
        ...
        use editor_core::diagnostic::{DiagnosticProvider, DiagnosticTag, Range, Severity};
    ...
    pub fn highlight_focused_view_elements(
        ...
        let pos = editor_core::match_brackets::find_matching_bracket(syntax, text, pos)?;
    ...
    pub fn render_bufferline(editor: &Editor, viewport: Rect, surface: &mut Surface) {
        ...
        let titles: Vec<Line<'static>> = editor
            ...
            .map(|(index, doc)| {
                ...
                if show_file_icon && let Some(path) = doc.path() {
...
fn workspace_trust_indicator_visible(editor: &Editor) -> bool {
    if editor.workspace_trust.implicit_level()
        == loader::workspace_trust::ImplicitTrustLevel::Insecure
    {
        return false;
    }
    let (_, doc) = view::current_ref!(editor);
    editor
        .workspace_trust
            ...
            MouseEventKind::Down(MouseButton::Left) => {
                ...
                if let Some((coords, view_id)) = gutter_coords_and_view(editor, row, column) {
                        ...
                        view.pos_at_visual_coords(doc, coords.row as u16, coords.col as u16, true)
                    {
                        ...
                        commands::dap_toggle_breakpoints_impl(cxt, path, &[line]);
                        return EventResult::Consumed(None);
                    }
                }
