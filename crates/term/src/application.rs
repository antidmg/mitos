...
impl Application {
            ...
            tokio::select! {
                ...
                Some(msg) = self.jobs.status_messages.recv() => {
                    let severity = match msg.severity{
                        event::status::Severity::Hint => Severity::Hint,
                        event::status::Severity::Info => Severity::Info,
                        event::status::Severity::Warning => Severity::Warning,
                        event::status::Severity::Error => Severity::Error,
                    };
    ...
    pub async fn handle_language_server_message(
        &mut self,
        call: lsp_client::Call,
        server_id: LanguageServerId,
    ) {
        use lsp_client::{Call, MethodCall, Notification};
        ...
        match call {
            Call::Notification(lsp_client::jsonrpc::Notification { method, params, .. }) => {
                let notification = match Notification::parse(&method, params) {
                    Ok(notification) => notification,
                    Err(lsp_client::Error::Unhandled) => {
                ...
                match notification {
                    Notification::Initialized => {
                        ...
                        event::dispatch(view::events::LanguageServerInitialized {
                            editor: &mut self.editor,
                            server_id,
                        });
                    }
                    Notification::PublishDiagnostics(params) => {
                        let uri = match editor_core::Uri::try_from(params.uri) {
                            Ok(uri) => uri,
                            Err(err) => {
                                log::error!("{err}");
                                return;
                        ...
                        let provider = editor_core::diagnostic::DiagnosticProvider::Lsp {
                            server_id,
                            identifier: None,
                        };
            ...
            Call::MethodCall(lsp_client::jsonrpc::MethodCall {
                method, params, id, ..
            }) => {
                let reply = match MethodCall::parse(&method, params) {
                    Err(lsp_client::Error::Unhandled) => {
                        ...
                        Err(lsp_client::jsonrpc::Error {
                            code: lsp_client::jsonrpc::ErrorCode::MethodNotFound,
                    ...
                    Err(err) => {
                        ...
                        Err(lsp_client::jsonrpc::Error {
                            code: lsp_client::jsonrpc::ErrorCode::ParseError,
    ...
    fn handle_show_document(
        ...
        let uri = match editor_core::Uri::try_from(uri) {
            Ok(uri) => uri,
            Err(err) => {
                log::error!("{err}");
        ...
        let action = if self.compositor.layer_count() == 1 {
            view::editor::Action::Replace
        } else {
            match take_focus {
                Some(true) => view::editor::Action::Replace,
                _ => view::editor::Action::Load,
            }
        };
