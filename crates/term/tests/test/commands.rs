...
async fn select_all_diagnostics_inside_selection() -> anyhow::Result<()> {
    ...
    {
        let (_view, doc) = view::current!(app.editor);
...
async fn test_quicklist_motion_pushes_pre_jump_location_to_jumplist() -> anyhow::Result<()> {
    ...
    let initial_cursor = 0;
    {
        ...
        doc.set_selection(view.id, editor_core::Selection::point(initial_cursor));
...
async fn test_failed_quicklist_motion_does_not_pollute_jumplist() -> anyhow::Result<()> {
    test_key_sequence(
        ...
        Some("]q"),
        Some(&|app| {
            let (view, _) = view::current_ref!(app.editor);
...
async fn test_jumplist_picker_quicklist_preserves_exact_selection() -> anyhow::Result<()> {
    ...
    let selection = editor_core::Selection::single(6, 10);
    {
        let (view, doc) = view::current!(app.editor);
        doc.set_selection(view.id, selection.clone());
        view.push_jump(doc, (doc.id(), selection.clone()));
    }
...
#[tokio::test(flavor = "multi_thread")]
async fn test_syntax_symbol_quicklist_preserves_exact_declaration_selection() -> anyhow::Result<()>
{
    let dir = tempfile::tempdir()?;
    ...
    test_key_sequence(
        ...
        Some(&|app| {
            ...
            assert_eq!(
                doc.selection(view.id).primary().fragment(text),
                "fn banana() {}"
            );
        }),
        false,
...
async fn global_search_with_multibyte_chars() -> anyhow::Result<()> {
    // Assert that `term::commands::global_search` handles multibyte characters correctly.
    test((
        indoc! {"\
            // Hello world!
            // #[|
