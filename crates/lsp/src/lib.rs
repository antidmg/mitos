...
pub mod util {
    ...
    pub fn diagnostic_to_lsp_diagnostic(
        doc: &Rope,
        diag: &editor_core::diagnostic::Diagnostic,
        ...
        let new_tags: Vec<_> = diag
            ...
            .iter()
            .map(|tag| match tag {
                editor_core::diagnostic::DiagnosticTag::Unnecessary => {
                    lsp::DiagnosticTag::UNNECESSARY
                }
                editor_core::diagnostic::DiagnosticTag::Deprecated => {
    ...
    pub fn generate_transaction_from_edits(
        ...
        if edits.len() == 1 {
            ...
            if is_document_replacement {
                ...
                return editor_core::diff::compare_ropes(doc, &new_text);
            }
        }
        ...
        let mut last_end = 0;
        let mut changes: Vec<(usize, usize, Option<editor_core::Tendril>)> =
...
pub struct Registry {
    ...
    syn_loader: Arc<ArcSwap<editor_core::syntax::Loader>>,
    pub incoming: SelectAll<UnboundedReceiverStream<(LanguageServerId, Call)>>,
    pub file_event_handler: file_event::Handler,
}

impl Registry {
    pub fn new(syn_loader: Arc<ArcSwap<editor_core::syntax::Loader>>) -> Self {
        Self {
    ...
    /// See view::editor::Editor::refresh_language_servers
...
fn start_client(
    ...
    let (workspace, workspace_is_cwd) = loader::find_workspace();
...
pub fn find_lsp_workspace(
    ...
    let mut file = if file.is_absolute() {
        ...
        let current_dir = stdx::env::current_working_dir();
