//! `event` contains systems that allow (often async) communication between
...
//! to view in the future if we manage to detach the compositor from its rendering backend.
