//! `mitos-event` contains systems that allow (often async) communication between
...
//! hooks that may not have access to all application data (for example in mitos-view).
//! This include the ability to control rendering ([`lock_frame`], [`request_redraw`]) and
//! display status messages ([`status`]).
//!
//! Hooks declared in mitos-term can furthermore dispatch synchronous jobs to be run on the
//! main loop (including access to the compositor). Ideally that queue will be moved
//! to mitos-view in the future if we manage to detach the compositor from its rendering backend.
...
macro_rules! register_hook {
    ...
    (move |$event:ident: &mut $event_ty: ident<$($lt: lifetime),*>| $body: expr) => {
        ...
        unsafe {
            ...
            // If we want to be pedantic we can simply compile mitos with
