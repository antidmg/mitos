...
Mitos currently supports one-way key remapping through a simple TOML configuration
...
* Static commands: commands like `move_char_right` which are usually bound to
  ...
  in [`crates/term/src/commands.rs`](https://github.com/matoous/mitos/blob/master/crates/term/src/commands.rs)
...
* Typable commands: commands that can be executed from command mode (`:`), for
  ...
  the source code at [`crates/term/src/commands/typed.rs`](https://github.com/matoous/mitos/blob/master/crates/term/src/commands/typed.rs).
...
To remap keys, create a `config.toml` file in your `mitos` configuration
directory (default `~/.config/mitos` on Linux systems) with a structure like
this:
...
C-o = ":open ~/.config/mitos/config.toml" # Maps Ctrl-o to opening of the mitos config file
