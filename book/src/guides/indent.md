...
By default, Mitos uses the `hybrid` indentation heuristic. This means that
...
When Mitos is inserting a new line through `o`, `O`, or `<ret>`, to determine
...
To address this case, `@extend` tells mitos to "extend" the captured node's span
