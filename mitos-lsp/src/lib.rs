...
pub use mitos_lsp_types as lsp;

use futures_util::stream::select_all::SelectAll;
use mitos_core::syntax::config::{
...
pub use mitos_core::diagnostic::LanguageServerId;
...
pub enum Error {
    ...
    Unhandled,
    #[error(transparent)]
    ExecutableNotFound(#[from] mitos_stdx::env::ExecutableNotFoundError),
    #[error(transparent)]
...
pub mod util {
    use super::*;
    use mitos_core::line_ending::{line_end_byte_index, line_end_char_index};
    use mitos_core::snippets::{RenderedSnippet, Snippet, SnippetRenderCtx};
    use mitos_core::{chars, RopeSlice};
    use mitos_core::{diagnostic::NumberOrString, Range, Rope, Selection, Tendril, Transaction};
    ...
    pub fn diagnostic_to_lsp_diagnostic(
        doc: &Rope,
        diag: &mitos_core::diagnostic::Diagnostic,
        ...
        let new_tags: Vec<_> = diag
            ...
            .iter()
            .map(|tag| match tag {
                mitos_core::diagnostic::DiagnosticTag::Unnecessary => {
                    lsp::DiagnosticTag::UNNECESSARY
                }
                mitos_core::diagnostic::DiagnosticTag::Deprecated => lsp::DiagnosticTag::DEPRECATED,
            })
            .collect();
    ...
    pub fn lsp_pos_to_pos(
        ...
        //
        // FIXME: Mitos does not fully comply with the LSP spec for line terminators.
        // The LSP standard requires that line terminators are ['\n', '\r\n', '\r'].
        // Without the unicode-linebreak feature disabled, the `\r` terminator is not handled by mitos.
        // With the unicode-linebreak feature, mitos recognizes multiple extra line break chars
    ...
    pub fn generate_transaction_from_edits(
        ...
        if edits.len() == 1 {
            ...
            if is_document_replacement {
                ...
                return mitos_core::diff::compare_ropes(doc, &new_text);
            }
        }
        ...
        let mut last_end = 0;
        let mut changes: Vec<(usize, usize, Option<mitos_core::Tendril>)> =
...
pub struct Registry {
    ...
    syn_loader: Arc<ArcSwap<mitos_core::syntax::Loader>>,
    pub incoming: SelectAll<UnboundedReceiverStream<(LanguageServerId, Call)>>,
    pub file_event_handler: file_event::Handler,
}

impl Registry {
    pub fn new(syn_loader: Arc<ArcSwap<mitos_core::syntax::Loader>>) -> Self {
        Self {
    ...
    /// See mitos_view::editor::Editor::refresh_language_servers
...
fn start_client(
    ...
    let (workspace, workspace_is_cwd) = mitos_loader::find_workspace();
...
pub fn find_lsp_workspace(
    ...
    let mut file = if file.is_absolute() {
        ...
        let current_dir = mitos_stdx::env::current_working_dir();
