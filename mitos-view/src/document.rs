...
use mitos_core::auto_pairs::AutoPairs;
use mitos_core::chars::char_is_word;
use mitos_core::command_line::Token;
use mitos_core::diagnostic::DiagnosticProvider;
use mitos_core::doc_formatter::TextFormat;
use mitos_core::encoding::Encoding;
use mitos_core::snippets::{ActiveSnippet, SnippetRenderCtx};
use mitos_core::syntax::config::LanguageServerFeature;
use mitos_core::text_annotations::{InlineAnnotation, Overlay};
use mitos_event::TaskController;
use mitos_lsp::util::lsp_pos_to_pos;
use mitos_stdx::faccess::{copy_metadata, readonly};
use mitos_vcs::{DiffHandle, DiffProviderRegistry};
...
/// wants them but it shouldn't be Mitos who decides that so we use the most precise positioning.
///
...
use mitos_lsp::{lsp, Client, LanguageServerId, LanguageServerName};
use mitos_stdx::Url;
...
impl Document {
    ...
    pub fn format(
        ...
        if let Some((fmt_cmd, fmt_args)) = self
            ...
            .and_then(|formatter| {
                Some((
                    mitos_stdx::env::which(&formatter.command).ok()?,
                    &formatter.args,
                ))
            })
        {
            ...
            let formatting_future = async move {
                ...
                Ok(mitos_core::diff::compare_ropes(&text, &Rope::from(str)))
            };
        ...
        let fut = async move {
            let edits = request
                ...
                .unwrap_or_default();
            Ok(mitos_lsp::util::generate_transaction_from_edits(
                &text,
                edits,
                offset_encoding,
            ))
    ...
    > {
        ...
        let path = match path {
            Some(path) => mitos_stdx::path::canonicalize(path),
            None => {
        ...
        let future = async move {
            ...
            let is_hardlink = mitos_stdx::faccess::hardlink_count(&write_path).unwrap_or(2) > 1;
    ...
    pub fn reload(
        ...
        // of the encoding.
        let transaction = mitos_core::diff::compare_ropes(self.text(), &rope);
    ...
    pub fn set_path(&mut self, path: Option<&Path>) {
        let path = path.map(mitos_stdx::path::canonicalize);
    ...
    pub fn set_selection(&mut self, view_id: ViewId, selection: Selection) {
        ...
        mitos_event::dispatch(SelectionDidChange {
            doc: self,
            view: view_id,
        })
    }
    ...
    fn apply_impl(
        ...
        mitos_event::dispatch(DocumentDidChange {
            doc: self,
            view: view_id,
            old_text: &old_doc,
            changes,
    ...
    pub fn language_servers(&self) -> impl Iterator<Item = &mitos_lsp::Client> {
    ...
    ) -> impl Iterator<Item = &mitos_lsp::Client> {
        self.language_config().into_iter().flat_map(move |config| {
            config.language_servers.iter().filter_map(move |features| {
                let ls = &**self.language_servers.get(&features.name)?;
                if ls.is_initialized()
    ...
    pub fn uri(&self) -> Option<mitos_core::Uri> {
        Some(self.path()?.into())
    }
    ...
    pub fn relative_path(&self) -> Option<&Path> {
        self.relative_path
            .get_or_init(|| {
                self.path
                    .as_ref()
                    .map(|path| mitos_stdx::path::get_relative_path(path).to_path_buf())
            })
            .as_deref()
    }

    /// The workspace root for this document — the nearest ancestor that contains a `.git`, `.svn`,
    /// `.jj`, or `.mitos`. Falls back to the current working directory's workspace when the
    ...
    pub fn workspace_root(&self) -> &Path {
        self.workspace_root
            .get_or_init(|| match self.path.as_deref() {
                Some(p) => p
                    .parent()
                    .map(|dir| mitos_loader::find_workspace_in(dir).0)
                    .unwrap_or_else(|| mitos_loader::find_workspace().0),
                None => mitos_loader::find_workspace().0,
            })
    }
    ...
    pub fn position(
        &self,
        view_id: ViewId,
        offset_encoding: mitos_lsp::OffsetEncoding,
    ) -> lsp::Position {
        let text = self.text();

        mitos_lsp::util::pos_to_lsp_pos(
            text,
    ...
    pub fn lsp_diagnostic_to_diagnostic(
        ...
        diagnostic: &mitos_lsp::lsp::Diagnostic,
        provider: DiagnosticProvider,
        offset_encoding: mitos_lsp::OffsetEncoding,
    ) -> Option<Diagnostic> {
        use mitos_core::diagnostic::{Range, Severity::*};
        ...
        use mitos_core::diagnostic::{DiagnosticTag, NumberOrString};
...
mod test {
    ...
    fn changeset_to_changes_ignore_line_endings() {
        use mitos_lsp::{lsp, Client, OffsetEncoding};
    ...
    fn test_line_ending() {
        assert_eq!(
            ...
            .text()
            .to_string(),
            mitos_core::NATIVE_LINE_ENDING.as_str()
        );
    }
    ...
    macro_rules! encode {
        ($name:ident, $label:expr, $label_override:expr) => {
            ...
            fn $name() {
                ...
                mitos_lsp::block_on(to_writer(&mut buf, (encoding, false), &text)).unwrap();
