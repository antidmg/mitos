...
use mitos_core::command_line::{ExpansionKind, Token, TokenKind, Tokenizer};
...
fn expand_variable(editor: &Editor, variable: Variable) -> Result<Cow<'static, str>> {
    ...
    match variable {
        ...
        Variable::CursorColumn => {
            ...
            let position = mitos_core::coords_at_pos(text, cursor);
        ...
        Variable::FilePathAbsolute => {
            let path = match doc.path() {
                ...
                None => mitos_stdx::env::current_working_dir(),
            }
            .to_string_lossy()
            .to_string();
            Ok(Cow::Owned(path))
        ...
        Variable::CurrentWorkingDirectory => Ok(std::borrow::Cow::Owned(
            mitos_stdx::env::current_working_dir()
                .to_string_lossy()
                .to_string(),
        )),
        Variable::WorkspaceDirectory => Ok(std::borrow::Cow::Owned(
            mitos_loader::find_workspace()
                .0
                .to_string_lossy()
                .to_string(),
        )),
