...
use mitos_core::Uri;
use mitos_event::register_hook;
use mitos_lsp::util::generate_transaction_from_edits;
use mitos_lsp::{lsp, LanguageServerId, OffsetEncoding};
...
pub enum ApplyEditErrorKind {
    DocumentChanged,
    FileNotFound,
    InvalidUrl(mitos_core::uri::UrlConversionError),
...
impl From<mitos_core::uri::UrlConversionError> for ApplyEditErrorKind {
    fn from(err: mitos_core::uri::UrlConversionError) -> Self {
...
impl Editor {
    ...
    pub fn handle_lsp_diagnostics(
        ...
        if let Some(doc) = doc {
            ...
            let doc = doc.id();
            mitos_event::dispatch(DiagnosticsDidChange { editor: self, doc });
        }
    }
