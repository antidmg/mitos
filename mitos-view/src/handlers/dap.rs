...
// general utils:
pub fn dap_pos_to_pos(doc: &mitos_core::Rope, line: usize, column: usize) -> Option<usize> {
...
pub fn jump_to_stack_frame(editor: &mut Editor, frame: &mitos_dap::StackFrame) {
    let path = if let Some(mitos_dap::Source {
        path: Some(ref path),
        ..
    }) = frame.source
    {
...
pub fn breakpoints_changed(
    ...
    let source_breakpoints = breakpoints
        .iter()
        .map(|breakpoint| mitos_dap::SourceBreakpoint {
...
impl Editor {
    pub async fn handle_debugger_message(
        &mut self,
        id: DebugAdapterId,
        payload: mitos_dap::Payload,
    ) -> bool {
        use mitos_dap::{events, Event};
