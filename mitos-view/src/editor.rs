...
use mitos_event::dispatch;
use mitos_loader::workspace_trust::{ImplicitTrustLevel, TrustQuery, WorkspaceTrust};
use mitos_vcs::DiffProviderRegistry;

use futures_util::stream::select_all::SelectAll;
use futures_util::StreamExt;
use mitos_lsp::{Call, LanguageServerId};
...
pub use mitos_core::diagnostic::Severity;
use mitos_core::{
...
use mitos_dap::{self as dap, registry::DebugAdapterId};
use mitos_lsp::lsp;
use mitos_stdx::path::canonicalize;
...
impl From<&WorkspaceTrustConfig> for mitos_loader::workspace_trust::Config {
    fn from(v: &WorkspaceTrustConfig) -> Self {
        Self {
            level: v.level.into(),
            prompt: v.prompt,
            trusted_globs: mitos_loader::workspace_trust::build_trusted_globs(&v.trusted),
        }
    }
}
...
pub fn get_terminal_provider() -> Option<TerminalConfig> {
    use mitos_stdx::env::binary_exists;
...
pub fn get_terminal_provider() -> Option<TerminalConfig> {
    use mitos_stdx::env::{binary_exists, env_var_is_set};
...
pub struct Editor {
    ...
    pub language_servers: mitos_lsp::Registry,
...
impl Editor {
    pub fn new(
        ...
        let language_servers = mitos_lsp::Registry::new(syn_loader.clone());
    ...
    pub fn refresh_config(&mut self, old_config: &Config) {
        ...
        self._refresh();
        mitos_event::dispatch(crate::events::ConfigDidChange {
            editor: self,
            old: old_config,
            new: &config,
        })
    ...
    pub fn move_path(&mut self, old_path: &Path, new_path: &Path) -> io::Result<()> {
        ...
        for language_server in language_servers {
            ...
            let edit = match mitos_lsp::block_on(request) {
        ...
        let language_servers = lang.as_ref().map_or_else(HashMap::default, |language| {
            self.language_servers
                ...
                .filter_map(|(lang, client)| match client {
                    ...
                    Err(err) => {
                        if let mitos_lsp::Error::ExecutableNotFound(err) = err {
    ...
    pub fn new_file_from_stdin(&mut self, action: Action) -> Result<DocumentId, Error> {
        ...
        doc.ensure_view_init(view.id);
        let transaction =
            mitos_core::Transaction::insert(doc.text(), doc.selection(view.id), stdin.into())
    ...
    pub fn open(&mut self, path: &Path, action: Action) -> Result<DocumentId, DocumentOpenError> {
        let path = mitos_stdx::path::canonicalize(path);
        ...
        let id = if let Some(id) = id {
            ...
            mitos_event::dispatch(DocumentDidOpen {
                editor: self,
                doc: id,
            });
    ...
    pub fn close_document(&mut self, doc_id: DocumentId, force: bool) -> Result<(), CloseError> {
        ...
        mitos_event::dispatch(DocumentDidClose { editor: self, doc });
    ...
    pub fn doc_diagnostics<'a>(
        language_servers: &'a mitos_lsp::Registry,
        diagnostics: &'a Diagnostics,
        document: &Document,
    ) -> impl Iterator<Item = mitos_core::Diagnostic> + 'a {
    ...
    pub async fn wait_event(&mut self) -> EditorEvent {
        ...
        loop {
            tokio::select! {
                ...
                _ = mitos_event::redraw_requested() => {
    ...
    pub fn set_cwd(&mut self, path: &Path) -> std::io::Result<()> {
        self.last_cwd = mitos_stdx::env::set_current_working_dir(path)?;
