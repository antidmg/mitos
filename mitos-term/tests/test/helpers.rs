...
use anyhow::bail;
use mitos_core::{diagnostic::Severity, test, Selection, Transaction};
use mitos_loader::workspace_trust::WorkspaceTrust;
use mitos_term::{application::Application, args::Args, config::Config, keymap::merge_keys};
use mitos_view::{
    current_ref, doc,
...
impl LineFeedHandling {
    ...
    pub fn apply(&self, text: &str) -> String {
        let line_end = match self {
            LineFeedHandling::Native => mitos_core::NATIVE_LINE_ENDING,
...
pub async fn test_key_sequence_with_input_text<T: Into<TestCase>>(
    ...
    let (view, doc) = mitos_view::current!(app.editor);
...
pub fn test_syntax_loader(overrides: Option<String>) -> mitos_core::syntax::Loader {
    let mut lang = mitos_loader::config::default_lang_config();

    if let Some(overrides) = overrides {
        let override_toml = toml::from_str(&overrides).unwrap();
        lang = mitos_loader::merge_toml_values(lang, override_toml, 3);
    }

    mitos_core::syntax::Loader::new(lang.try_into().unwrap()).unwrap()
}
...
pub fn test_config() -> Config {
    Config {
        ...
        keys: mitos_term::keymap::default(),
        ..Default::default()
    }
}

pub fn test_editor_config() -> mitos_view::editor::Config {
    mitos_view::editor::Config {
        lsp: LspConfig {
            enable: false,
            ..Default::default()
        },
...
pub struct AppBuilder {
    args: Args,
    config: Config,
    syn_loader: mitos_core::syntax::Loader,
...
impl AppBuilder {
    ...
    pub fn with_file<P: Into<PathBuf>>(
        mut self,
        path: P,
        pos: Option<mitos_core::Position>,
    ) -> Self {
        self.args
            .files
    ...
    pub fn with_config(mut self, mut config: Config) -> Self {
        let keys = replace(&mut config.keys, mitos_term::keymap::default());
    ...
    pub fn with_lang_loader(mut self, syn_loader: mitos_core::syntax::Loader) -> Self {
