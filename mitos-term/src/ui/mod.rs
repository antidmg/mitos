        ...
        move |cx: &mut crate::compositor::Context, input: &str, event: PromptEvent| {
            match event {
                ...
                PromptEvent::Update | PromptEvent::Validate => {
                    ...
                    let is_crlf = doc!(cx.editor).line_ending == mitos_core::LineEnding::Crlf;
                                    ...
                                    let call: job::Callback = Callback::EditorCompositor(Box::new(
                                        move |_editor: &mut Editor, compositor: &mut Compositor| {
                                            ...
                                            let popup = Popup::new("invalid-regex", contents)
                                                .position(Some(mitos_core::Position::new(
...
pub fn file_explorer(root: PathBuf, editor: &Editor) -> Result<FileExplorer, std::io::Error> {
    ...
    let picker = Picker::new(
        ...
        move |cx, (path, is_dir): &(PathBuf, bool), action| {
            if *is_dir {
                let new_root = mitos_stdx::path::normalize(path);
...
pub mod completers {
    ...
    use mitos_core::command_line::{self, Tokenizer};
    use mitos_core::fuzzy::fuzzy_match;
    use mitos_core::syntax::config::LanguageServerFeature;
    use mitos_view::document::SCRATCH_BUFFER_NAME;
    use mitos_view::theme;
    use mitos_view::{editor::Config, Editor};
    ...
    pub fn theme(_editor: &Editor, input: &str) -> Vec<Completion> {
        let mut names = theme::Loader::read_names(&mitos_loader::config_dir().join("themes"));
        for rt_dir in mitos_loader::runtime_dirs() {
        ...
        F: Fn(&ignore::DirEntry) -> FileMatch,
    {
        ...
        let is_tilde = input == "~";
        let path = mitos_stdx::path::expand_tilde(Path::new(input));
        ...
        let (dir, file_name) = if input.ends_with(std::path::MAIN_SEPARATOR) {
            ...
            let path = if is_period {
                ...
                match path.parent() {
                    ...
                    _ => Cow::Owned(mitos_stdx::env::current_working_dir()),
                }
            };
