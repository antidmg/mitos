...
pub struct Injector<T, D> {
    ...
    _redraw: mitos_event::RequestRedrawOnDrop,
}
...
impl<T: 'static + Send + Sync, D: 'static + Send + Sync> Picker<T, D> {
    pub fn stream(
        ...
        let matcher = Nucleo::new(
            Config::DEFAULT,
            Arc::new(mitos_event::request_redraw),
            None,
            matcher_columns,
        );
    ...
    pub fn with_dynamic_query(
        ...
        mitos_event::send_blocking(&handler, event);
            ...
            PathOrId::Path(path) => {
                ...
                if self.preview_cache.contains_key(path) {
                    ...
                    if matches!(preview, CachedPreview::Document(doc) if doc.syntax().is_none()) {
                        mitos_event::send_blocking(&self.preview_highlight_handler, path.clone());
                    }
