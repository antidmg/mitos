...
use mitos_core::snippets::{ActiveSnippet, RenderedSnippet, Snippet};
use mitos_core::{self as core, chars, fuzzy::MATCHER, Change, Transaction};
use mitos_lsp::{lsp, util, OffsetEncoding};
use mitos_view::{
...
use mitos_view::{graphics::Rect, Document, Editor};
use nucleo::{
...
pub struct Completion {
    ...
    filter: String,
    // TODO: move to mitos-view/central handler struct in the future
...
impl Completion {
    ...
    fn resolve_completion_item(
        language_server: &mitos_lsp::Client,
        ...
        let response = mitos_lsp::block_on(future);
        match response {
            Ok(item) => Some(item),
            Err(err) => {
