...
impl EditorView {
    ...
    pub fn doc_diagnostics_highlights_into(
        ...
        use mitos_core::diagnostic::{DiagnosticTag, Range, Severity};
    ...
    pub fn highlight_focused_view_elements(
        ...
        let pos = mitos_core::match_brackets::find_matching_bracket(syntax, text, pos)?;
    ...
    fn handle_keymap_event(
        ...
        let mut execute_command = |command: &commands::MappableCommand| {
            command.execute(cxt);
            mitos_event::dispatch(PostCommand { command, cx: cxt });

            let current_mode = cxt.editor.mode();
            if current_mode != last_mode {
                mitos_event::dispatch(OnModeSwitch {
                    old_mode: last_mode,
...
fn workspace_trust_indicator_visible(editor: &Editor) -> bool {
    if editor.workspace_trust.implicit_level()
        == mitos_loader::workspace_trust::ImplicitTrustLevel::Insecure
    {
        return false;
    }
    let (_, doc) = mitos_view::current_ref!(editor);
    editor
        .workspace_trust
...
impl Component for EditorView {
    ...
    fn render(&mut self, area: Rect, surface: &mut Surface, cx: &mut Context) {
        ...
        use mitos_view::editor::BufferLine;
        ...
        if let Some((status_msg, severity)) = &cx.editor.status_msg {
            ...
            use mitos_view::editor::Severity;
        ...
        if area.width.saturating_sub(status_msg_width as u16) > key_width {
            ...
            if restricted {
                let style = style
                    .fg(mitos_view::graphics::Color::Yellow)
