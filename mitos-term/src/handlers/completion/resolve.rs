...
use mitos_event::{send_blocking, AsyncHook, TaskController, TaskHandle};
...
impl ResolveRequest {
    async fn execute(self, cancel: TaskHandle) {
        ...
        let Some(resolved_item) = mitos_event::cancelable_future(future, cancel).await else {
            return;
        };
