...
use mitos_core::Uri;
use mitos_event::{cancelable_future, register_hook, send_blocking};
use mitos_lsp::{lsp, LanguageServerId};
use mitos_view::document::Mode;
use mitos_view::events::{
    DiagnosticsDidChange, DocumentDidChange, DocumentDidOpen, LanguageServerInitialized,
};
use mitos_view::handlers::diagnostics::DiagnosticEvent;
use mitos_view::handlers::lsp::{PullAllDocumentsDiagnosticsEvent, PullDiagnosticsEvent};
use mitos_view::handlers::Handlers;
use mitos_view::{DocumentId, Editor};
...
impl mitos_event::AsyncHook for PullDiagnosticsHandler {
...
impl mitos_event::AsyncHook for PullAllDocumentsDiagnosticHandler {
        ...
        loop {
            match cancelable_future(futures.next(), &cancel).await {
                ...
                Some(Some((Err(err), DiagnosticProvider::Lsp { server_id, .. }, _))) => {
                    let parsed_cancellation_data = if let mitos_lsp::Error::Rpc(error) = err {
