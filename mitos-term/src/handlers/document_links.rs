...
use mitos_core::{syntax::config::LanguageServerFeature, Assoc};
...
impl mitos_event::AsyncHook for DocumentLinksHandler {
        ...
        .filter_map(|language_server| {
            ...
            Some(async move {
                ...
                let links: Vec<_> = links
                    .into_iter()
                    .filter_map(|link| {
                        let start = mitos_lsp::util::lsp_pos_to_pos(
                            &text,
                            link.range.start,
                            offset_encoding,
                        )?;
                        let end = mitos_lsp::util::lsp_pos_to_pos(
                            &text,
                            link.range.end,
                            offset_encoding,
                        )?;
...
pub(super) fn register_hooks(handlers: &Handlers) {
    ...
    register_hook!(move |event: &mut DocumentDidChange<'_>| {
        ...
        if !event.ghost_transaction {
            ...
            mitos_event::send_blocking(&tx, DocumentLinksEvent(event.doc.id()));
        }
