...
use mitos_core::{syntax::config::LanguageServerFeature, text_annotations::InlineAnnotation};
use mitos_event::{cancelable_future, register_hook};
use mitos_lsp::lsp;
use mitos_view::{
...
impl mitos_event::AsyncHook for DocumentColorsHandler {
            ...
            async move {
                let colors: Vec<_> = future
                    ...
                    .filter_map(|color_info| {
                        let pos = mitos_lsp::util::lsp_pos_to_pos(
                            &text,
    ...
    register_hook!(move |event: &mut DocumentDidChange<'_>| {
        ...
        let apply_color_swatch_changes = |annotations: &mut Vec<InlineAnnotation>| {
            event.changes.update_positions(
                annotations
                    .iter_mut()
                    .map(|annotation| (&mut annotation.char_idx, mitos_core::Assoc::After)),
            );
        };
        ...
        if !event.ghost_transaction {
            ...
            mitos_event::send_blocking(&tx, DocumentColorsEvent(event.doc.id()));
        }
