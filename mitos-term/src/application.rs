...
use mitos_core::{diagnostic::Severity, pos_at_coords, syntax, Range, Selection};
use mitos_lsp::{
    lsp::{self, notification::Notification},
    util::lsp_range_to_range,
    LanguageServerId, LspProgressMap,
};
use mitos_stdx::path::get_relative_path;
use mitos_view::{
    align_view,
...
fn setup_integration_logging() {
    let level = std::env::var("MITOS_LOG_LEVEL")
...
impl Application {
    pub fn new(
        ...
        workspace_trust: mitos_loader::workspace_trust::WorkspaceTrust,
    ) -> Result<Self, Error> {
        #[cfg(feature = "integration")]
        setup_integration_logging();

        use mitos_view::editor::Action;

        let mut theme_parent_dirs = vec![mitos_loader::config_dir()];
        theme_parent_dirs.extend(mitos_loader::runtime_dirs().iter().cloned());
        ...
        if args.load_tutor {
            let path = mitos_loader::runtime_file(Path::new("tutor"));
            ...
            tokio::select! {
                ...
                Some(msg) = self.jobs.status_messages.recv() => {
                    let severity = match msg.severity{
                        mitos_event::status::Severity::Hint => Severity::Hint,
                        mitos_event::status::Severity::Info => Severity::Info,
                        mitos_event::status::Severity::Warning => Severity::Warning,
                        mitos_event::status::Severity::Error => Severity::Error,
                    };
                    // TODO: show multiple status messages at once to avoid clobbering
                    self.editor.status_msg = Some((msg.message, severity));
                    mitos_event::request_redraw();
                }
    ...
    fn refresh_config(&mut self) {
        let mut refresh_config = || -> Result<(), Error> {
            ...
            let lang_loader = mitos_core::config::user_lang_loader(&self.editor.workspace_trust)?;
    ...
    pub async fn handle_signals(&mut self, signal: i32) -> bool {
        match signal {
            signal::SIGTSTP => {
                ...
                // SAFETY:
                //
                // - mitos must have permissions to send signals to all processes in its signal
                ...
                let res = unsafe {
                    ...
                    // as mentioned above, the terminal will get stuck if `mitos` was spawned from
                    // an external process and that process waits for `mitos` to complete. This may
    ...
    pub async fn handle_language_server_message(
        ...
        use mitos_lsp::{Call, MethodCall, Notification};
        ...
        match call {
            Call::Notification(mitos_lsp::jsonrpc::Notification { method, params, .. }) => {
                let notification = match Notification::parse(&method, params) {
                    Ok(notification) => notification,
                    Err(mitos_lsp::Error::Unhandled) => {
                ...
                match notification {
                    Notification::Initialized => {
                        ...
                        mitos_event::dispatch(mitos_view::events::LanguageServerInitialized {
                            editor: &mut self.editor,
                            server_id,
                        });
                    }
                    Notification::PublishDiagnostics(params) => {
                        let uri = match mitos_core::Uri::try_from(params.uri) {
                            Ok(uri) => uri,
                            Err(err) => {
                                log::error!("{err}");
                                return;
                        ...
                        let provider = mitos_core::diagnostic::DiagnosticProvider::Lsp {
                            server_id,
                            identifier: None,
                        };
                    ...
                    Notification::Exit => {
                        ...
                        // LSPs may produce diagnostics for files that haven't been opened in mitos,
                        ...
                        mitos_event::dispatch(mitos_view::events::LanguageServerExited {
                            editor: &mut self.editor,
                            server_id,
                        });
            ...
            Call::MethodCall(mitos_lsp::jsonrpc::MethodCall {
                method, params, id, ..
            }) => {
                let reply = match MethodCall::parse(&method, params) {
                    Err(mitos_lsp::Error::Unhandled) => {
                        ...
                        Err(mitos_lsp::jsonrpc::Error {
                            code: mitos_lsp::jsonrpc::ErrorCode::MethodNotFound,
                    ...
                    Err(err) => {
                        ...
                        Err(mitos_lsp::jsonrpc::Error {
                            code: mitos_lsp::jsonrpc::ErrorCode::ParseError,
                    ...
                    Ok(MethodCall::ApplyWorkspaceEdit(params)) => {
                        ...
                        if language_server.is_initialized() {
                            ...
                            Err(mitos_lsp::jsonrpc::Error {
                                code: mitos_lsp::jsonrpc::ErrorCode::InvalidRequest,
    ...
    fn handle_show_document(
        ...
        let uri = match mitos_core::Uri::try_from(uri) {
            Ok(uri) => uri,
            Err(err) => {
                log::error!("{err}");
        ...
        let action = if self.compositor.layer_count() == 1 {
            mitos_view::editor::Action::Replace
        } else {
            match take_focus {
                Some(true) => mitos_view::editor::Action::Replace,
                _ => mitos_view::editor::Action::Load,
            }
        };
    ...
    fn restore_term(&mut self) -> std::io::Result<()> {
        use mitos_view::graphics::CursorKind;
        self.terminal
            .backend_mut()
