...
pub use lsp::*;
use mitos_event::status;
use mitos_stdx::{
    path::{self, find_paths},
    rope::{self, RopeSliceExt},
};
use mitos_vcs::{FileChange, Hunk};
pub use syntax::*;
use tui::{
    text::{Line, Span},
    widgets::Cell,
...
impl Context<'_> {
    ...
    pub fn callback<T, F>(
        &mut self,
        call: impl Future<Output = mitos_lsp::Result<T>> + 'static + Send,
        callback: F,
    ) where
        T: Send + 'static,
...
use mitos_view::{align_view, Align};
...
impl std::str::FromStr for MappableCommand {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        if let Some(suffix) = s.strip_prefix(':') {
            ...
            mitos_view::input::parse_macro(suffix).map(|keys| Self::Macro {
                name: s.to_string(),
                keys,
            })
        } else {
...
use mitos_core::movement::{move_horizontally, move_vertically};
...
fn align_selections(cx: &mut Context) {
    use mitos_core::visual_coords_at_pos;
...
fn selection_overlaps_document_link(
    selection: &Range,
    link: &mitos_view::document::DocumentLink,
) -> bool {
...
fn resolve_document_link_request(
    editor: &Editor,
    link: &mitos_view::document::DocumentLink,
) -> Option<impl Future<Output = mitos_lsp::Result<mitos_lsp::lsp::DocumentLink>> + Send + 'static>
{
...
fn copy_selection_on_line(cx: &mut Context, direction: Direction) {
    use mitos_core::{pos_at_visual_coords, visual_coords_at_pos};
...
fn global_search(cx: &mut Context) {
    ...
    impl FileResult<'_> {
        fn new(path: &Path, line_start: usize, line_end: usize) -> Self {
            Self {
                path: mitos_stdx::path::get_relative_path(path.to_path_buf()),
                line_start,
                line_end,
            }
        }
    ...
    struct GlobalSearchConfig {
        smart_case: bool,
        file_picker_config: mitos_view::editor::FilePickerConfig,
    ...
    let get_files = |query: &str,
        ...
        let search_root = mitos_stdx::env::current_working_dir();
        ...
        async move {
            ...
            WalkBuilder::new(search_root)
                ...
                .add_custom_ignore_filename(mitos_loader::config_dir().join("ignore"))
...
impl PathStyleConfig {
    fn new(theme: &mitos_view::Theme) -> Self {
        Self {
...
fn buffer_picker(cx: &mut Context) {
    ...
    let new_meta = |doc: &Document| BufferMeta {
        ...
        path: doc
            ...
            .map(mitos_stdx::path::get_relative_path),
...
fn changed_file_picker(cx: &mut Context) {
    ...
    let trust_full = cx
        ...
        .workspace_trust
        .query(
            &mitos_loader::find_workspace_in(&cwd).0,
...
fn continued_line_comment_token<'a>(
    doc: &'a Document,
    loader: &'a mitos_core::syntax::Loader,
    text: RopeSlice,
    line_num: usize,
    byte_pos: usize,
) -> Option<&'a str> {
...
pub mod insert {
    ...
    use mitos_core::auto_pairs;
    use mitos_view::editor::SmartTabConfig;
    ...
    pub fn insert_char(cx: &mut Context, c: char) {
        ...
        mitos_event::dispatch(PostInsertChar { c, cx });
    }
...
fn format_selections(cx: &mut Context) {
    use mitos_lsp::{lsp, util::range_to_lsp_range};
    ...
    tokio::spawn(async move {
        match future.await {
            Ok(Some(res)) => {
                let transaction =
                    mitos_lsp::util::generate_transaction_from_edits(&text, res, offset_encoding);
...
where
    F: Fn(&mitos_core::Syntax, RopeSlice, Selection) -> Selection + 'static,
{
    let motion = move |editor: &mut Editor| {
        let (view, doc) = current!(editor);
...
fn shell_impl(shell: &[String], cmd: &str, input: Option<Rope>) -> anyhow::Result<Tendril> {
    tokio::task::block_in_place(|| mitos_lsp::block_on(shell_impl_async(shell, cmd, input)))
}
    ...
    let output = if let Some(mut stdin) = process.stdin.take() {
        let input_task = tokio::spawn(async move {
            if let Some(input) = input {
                mitos_view::document::to_writer(&mut stdin, (encoding::UTF_8, false), &input)
                    .await?;
            }
            anyhow::Ok(())
        });
...
fn replay_macro(cx: &mut Context) {
    ...
    {
        match mitos_view::input::parse_macro(&keys) {
            Ok(keys) => keys,
            Err(err) => {
