...
use mitos_core::syntax::config::{DebugConfigCompletion, DebugTemplate};
use mitos_dap::{self as dap, requests::TerminateArguments};
use mitos_lsp::block_on;
use mitos_view::editor::Breakpoint;
...
use mitos_view::handlers::dap::{breakpoints_changed, jump_to_stack_frame, select_thread_id};
...
fn dap_callback<T, F>(
    jobs: &mut Jobs,
    call: impl Future<Output = mitos_dap::Result<serde_json::Value>> + 'static + Send,
    callback: F,
) where
...
pub fn dap_start_impl(
    ...
    if !cx
        .editor
        .workspace_trust
        .query(&workspace, mitos_loader::workspace_trust::TrustQuery::Dap)
        .is_trusted()
    {
    ...
    args.insert("cwd", to_value(mitos_stdx::env::current_working_dir())?);
