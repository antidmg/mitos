...
/// A wrapper around `lsp::Location` that swaps out the LSP URI for `mitos_core::Uri` and adds
...
pub(crate) fn code_actions_for_range(
    doc: &Document,
    range: mitos_core::Range,
    only: Option<Vec<CodeActionKind>>,
    trigger_kind: CodeActionTriggerKind,
) -> Vec<(
    impl Future<Output = Result<Option<Vec<CodeActionOrCommand>>, mitos_lsp::Error>>,
    LanguageServerId,
)> {
    let mut seen_language_servers = HashSet::new();
        ...
        .filter_map(|language_server| {
            ...
            let code_action_context = lsp::CodeActionContext {
                diagnostics: doc
                    ...
                    .iter()
                    .filter(|&diag| {
                        range.overlaps(&mitos_core::Range::new(diag.range.start, diag.range.end))
                    })
...
fn code_action_on_save_step(
    ...
    let build_request = move |editor: &mut Editor| -> Option<Job> {
        ...
        let full_range = mitos_core::Range::new(0, doc.text().len_chars());
...
/// several edits.
type ResolveFuture =
    std::pin::Pin<Box<dyn Future<Output = Result<lsp::CodeAction, mitos_lsp::Error>> + Send>>;
enum ResolveStep {
...
fn goto_impl(editor: &mut Editor, compositor: &mut Compositor, locations: Vec<Location>) {
    let cwdir = mitos_stdx::env::current_working_dir();
...
where
    ...
    F: Future<Output = mitos_lsp::Result<Option<lsp::GotoDefinitionResponse>>> + 'static + Send,
{
...
pub fn rename_symbol(cx: &mut Context) {
    fn get_prefill_from_word_boundary(editor: &Editor) -> String {
        ...
        if primary_selection.len() > 1 {
            ...
            use mitos_core::textobject::{textobject_word, TextObject};
...
fn compute_inlay_hints_for_view(
    ...
    let range = mitos_lsp::util::range_to_lsp_range(
        doc_text,
        mitos_core::Range::new(first_char_in_range, last_char_in_range),
    ...
    let callback = super::make_job_callback(
        ...
        move |editor, _compositor, response: Option<Vec<lsp::InlayHint>>| {
            ...
            for hint in hints {
                let char_idx =
                    match mitos_lsp::util::lsp_pos_to_pos(doc_text, hint.position, offset_encoding)
                    {
                        Some(pos) => pos,
