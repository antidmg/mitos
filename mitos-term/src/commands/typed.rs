...
use mitos_core::command_line::{Args, Flag, Signature, Token, TokenKind};
use mitos_core::fuzzy::fuzzy_match;
use mitos_core::indent::MAX_INDENT;
use mitos_core::line_ending;
use mitos_stdx::path::home_dir;
use mitos_view::document::{read_to_string, DEFAULT_LANGUAGE_NAME};
use mitos_view::editor::{CloseError, ConfigEvent};
use mitos_view::expansion;
...
fn open_impl(cx: &mut compositor::Context, args: Args, action: Action) -> anyhow::Result<()> {
    for arg in args {
        ...
        let path = mitos_stdx::path::expand_tilde(path);
...
fn set_line_ending(
    ...
    let transaction = Transaction::change(
        ...
        doc.text().lines().filter_map(|line| {
            pos += line.len_chars();
            match mitos_core::line_ending::get_line_ending(&line) {
...
fn parse_first_arg_as_dir(args: &Args, last_cwd: Option<PathBuf>) -> anyhow::Result<PathBuf> {
    match args.first().map(AsRef::as_ref) {
        ...
        Some(path) => Ok(mitos_stdx::path::expand_tilde(Path::new(path)).into_owned()),
        None => Ok(home_dir()?),
    }
}
...
fn apply_directory_change(cx: &mut compositor::Context, dir: &Path) -> anyhow::Result<()> {
    ...
    cx.editor.set_status(format!(
        ...
        mitos_stdx::env::current_working_dir().display()
    ));
...
fn push_directory(
    ...
    cx.editor
        .dir_stack
        .push_front(mitos_stdx::env::current_working_dir());
...
fn reload_all(cx: &mut compositor::Context, _args: Args, event: PromptEvent) -> anyhow::Result<()> {
    ...
    for (doc_id, view_ids) in docs_view_ids {
        ...
        let trust_full = cx
            ...
            .query(
                doc.workspace_root(),
                mitos_loader::workspace_trust::TrustQuery::Git,
            )
            .is_trusted();
            ...
            let call: job::Callback = Callback::EditorCompositor(Box::new(
                move |_editor: &mut Editor, compositor: &mut Compositor| {
                    let columns = [ui::PickerColumn::new(
                        "title",
                        |(_ls_id, command): &(_, mitos_lsp::lsp::Command), _| {
...
fn lsp_restart(cx: &mut compositor::Context, args: Args, event: PromptEvent) -> anyhow::Result<()> {
    ...
    for server in language_servers.iter() {
            ...
            .transpose()
        {
            ...
            // in the arguments.
            Err(mitos_lsp::Error::ExecutableNotFound(_))
...
fn tree_sitter_highlight_name(
    ...
    while highlighter.next_event_offset() <= byte {
        ...
        if event == mitos_core::syntax::HighlightEvent::Refresh {
            highlights.clear();
        }
...
fn tree_sitter_layers(
    ...
    let loader: &mitos_core::syntax::Loader = &cx.editor.syn_loader.load();
    let text = doc.text().slice(..);
    let cursor = doc.selection(view.id).primary().cursor(text);
    let byte = text.char_to_byte(cursor) as u32;
    let languages =
...
fn debug_eval(cx: &mut compositor::Context, args: Args, event: PromptEvent) -> anyhow::Result<()> {
    ...
    if let Some(debugger) = cx.editor.debug_adapters.get_active_client() {
        ...
        let response = mitos_lsp::block_on(debugger.eval(args.join(" "), Some(frame_id)))?;
...
fn reflow(cx: &mut compositor::Context, args: Args, event: PromptEvent) -> anyhow::Result<()> {
    ...
    let transaction = Transaction::change_by_selection(rope, selection, |range| {
        ...
        let reflowed_text = mitos_core::wrap::reflow_hard_wrap(&fragment, text_width);
...
fn tree_sitter_subtree(
    ...
    if let Some(syntax) = doc.syntax() {
        ...
        if let Some(selected_node) = syntax.descendant_for_byte_range(from, to) {
            ...
            mitos_core::syntax::pretty_print_tree(&mut contents, selected_node)?;
            contents.push_str("\n```");
...
fn open_config(
    ...
    cx.editor
        .open(&mitos_loader::config_file(), Action::Replace)?;
    Ok(())
}
...
fn open_workspace_config(
    ...
    cx.editor
        .open(&mitos_loader::workspace_config_file(), Action::Replace)?;
    Ok(())
}
...
fn open_log(cx: &mut compositor::Context, _args: Args, event: PromptEvent) -> anyhow::Result<()> {
    ...
    cx.editor.open(&mitos_loader::log_file(), Action::Replace)?;
    Ok(())
}
            ...
            move |editor: &mut Editor, compositor: &mut Compositor| {
                if !output.trim().is_empty() {
                    ...
                    let popup = Popup::new("shell", contents).position(Some(
                        mitos_core::Position::new(editor.cursor().0.unwrap_or_default().row, 2),
                    ));
...
fn yank_diagnostic(
    ...
    let diag: Vec<_> = doc
        .diagnostics()
        .iter()
        .filter(|d| primary.overlaps(&mitos_core::Range::new(d.range.start, d.range.end)))
...
fn read(cx: &mut compositor::Context, args: Args, event: PromptEvent) -> anyhow::Result<()> {
    ...
    let path = mitos_stdx::path::expand_tilde(PathBuf::from(filename.to_string()));
...
/// `Trust::Full`).
fn doc_trust_full(editor: &mitos_view::Editor) -> bool {
