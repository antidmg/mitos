...
use mitos_loader::merge_toml_values;
use mitos_view::{document::Mode, theme};
...
pub struct Config {
    ...
    pub editor: mitos_view::editor::Config,
}
...
impl Default for Config {
    fn default() -> Config {
        Config {
            theme: None,
            keys: keymap::default(),
            editor: mitos_view::editor::Config::default(),
        }
    }
}
...
impl Config {
        ...
        let res = match (global_config, local_config) {
            (Ok(global), Ok(local)) => {
                ...
                let editor = match (global.editor, local.editor) {
                    (None, None) => mitos_view::editor::Config::default(),
            ...
            (Ok(config), Err(_)) | (Err(_), Ok(config)) => {
                ...
                Config {
                    ...
                    editor: config.editor.map_or_else(
                        || Ok(mitos_view::editor::Config::default()),
    ...
    pub fn load_default() -> Result<Config, ConfigLoadError> {
        let global_config =
            fs::read_to_string(mitos_loader::config_file()).map_err(ConfigLoadError::Error)?;
        let local_config = fs::read_to_string(mitos_loader::workspace_config_file())
        ...
        // after the check.
        let trust = mitos_loader::workspace_trust::WorkspaceTrust::new(
            (&global_parsed.editor.workspace_trust).into(),
        );
        if trust
            .query_current(mitos_loader::workspace_trust::TrustQuery::LocalConfig)
            .is_trusted()
        {
