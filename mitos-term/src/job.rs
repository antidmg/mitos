use mitos_event::status::StatusMessage;
use mitos_event::{runtime_local, send_blocking};
...
impl Jobs {
    ...
    pub fn new() -> Self {
        ...
        let status_messages = mitos_event::status::setup();
        Self {
        ...
        if j.wait {
            ...
            tokio::spawn(async move {
                match j.future.await {
                    ...
                    Ok(None) => (),
                    Err(err) => mitos_event::status::report(err).await,
                }
            });
        }
    }
