...
use mitos_core::config::{default_lang_config, user_lang_config};
use mitos_loader::grammar::load_runtime_file;
use std::{
    collections::HashSet,
    io::{IsTerminal, Write},
};
...
pub fn general() -> std::io::Result<()> {
    ...
    let config_file = mitos_loader::config_file();
    let lang_file = mitos_loader::lang_config_file();
    let log_file = mitos_loader::log_file();
    let rt_dirs = mitos_loader::runtime_dirs();
...
pub fn languages_selection() -> std::io::Result<()> {
    let selection = mitos_loader::grammar::get_grammar_names().unwrap_or_default();
    languages(selection)
}
...
fn languages(selection: Option<HashSet<String>>) -> std::io::Result<()> {
    ...
    let trust = mitos_loader::workspace_trust::WorkspaceTrust::fully_trusted();
    ...
    let check_binary_with_name = |cmd: Option<(&str, &str)>| match cmd {
        Some((name, cmd)) => match mitos_stdx::env::which(cmd) {
...
fn probe_parser(grammar_name: &str) -> std::io::Result<()> {
    ...
    match mitos_loader::grammar::get_language(grammar_name) {
...
fn probe_protocols<'a, I: Iterator<Item = (&'a str, &'a str)> + 'a>(
    ...
    for (name, cmd) in server_cmds {
        let (diag, icon) = match mitos_stdx::env::which(cmd) {
