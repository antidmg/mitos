...
use mitos_loader::VERSION_AND_GIT_HASH;
use mitos_term::application::Application;
use mitos_term::args::Args;
use mitos_term::config::{Config, ConfigLoadError};
...
fn setup_logging(verbosity: u64) -> Result<()> {
    ...
    mitos_term::logging::init_file(level, &mitos_loader::log_file())?;
...
async fn main_impl() -> Result<i32> {
    ...
    mitos_loader::initialize_config_file(args.config_file.clone());
    mitos_loader::initialize_log_file(args.log_file.clone());
...
",
            ...
            mitos_loader::default_log_file().display(),
        );
        std::process::exit(0);
    }

    if args.display_version {
        println!("mitos {}", VERSION_AND_GIT_HASH);
        std::process::exit(0);
    }

    if args.health {
        if let Err(err) = mitos_term::health::print_health(args.health_arg) {
    ...
    if args.fetch_grammars {
        mitos_loader::grammar::fetch_grammars(args.strict)?;
        return Ok(0);
    }

    if args.build_grammars {
        mitos_loader::grammar::build_grammars(None, args.strict)?;
        return Ok(0);
    }
    ...
    if let Some(path) = &args.working_directory {
        mitos_stdx::env::set_current_working_dir(path)?;
    ...
    let workspace_trust =
        mitos_loader::workspace_trust::WorkspaceTrust::new((&config.editor.workspace_trust).into());

    let lang_loader =
        mitos_core::config::user_lang_loader(&workspace_trust).unwrap_or_else(|err| {
            eprintln!("{}", err);
            eprintln!("Press <ENTER> to continue with default language config");
            use std::io::Read;
            // This waits for an enter press.
            let _ = std::io::stdin().read(&mut []);
            mitos_core::config::default_lang_loader()
        });

    // TODO: use the thread local executor to spawn the application task separately from the work pool
    let mut app = Application::new(args, config, lang_loader, workspace_trust)
        .context("unable to start Mitos")?;
