...
impl Uri {
    // This clippy allow mirrors mitos_stdx::Url::from_file_path
    #[allow(clippy::result_unit_err)]
    pub fn to_url(&self) -> Result<mitos_stdx::Url, ()> {
        match self {
            Uri::File(path) => mitos_stdx::Url::from_file_path(path),
        }
    }
...
fn convert_url_to_uri(url: &mitos_stdx::Url) -> Result<Uri, UrlConversionErrorKind> {
    if url.scheme() == "file" {
        url.to_file_path()
            .map(|path| Uri::File(mitos_stdx::path::normalize(path).into()))
...
impl TryFrom<mitos_stdx::Url> for Uri {
    type Error = UrlConversionError;

    fn try_from(url: mitos_stdx::Url) -> Result<Self, Self::Error> {
