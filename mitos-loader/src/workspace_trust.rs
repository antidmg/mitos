//! Workspace trust.
//!
//! Mitos can load workspace-local configuration (`.mitos/`) and launch language servers, both of
...
//! A grant snapshots a hash of every file under `.mitos/`. If those files change later the
...
//! multiple concurrent mitos instances writing trust for *different* workspaces (different
...
//! bypasses the `.mitos/` hash pin (changes to local config are never re-checked) and it trusts any
...
pub enum TrustQuery {
    ...
    Dap,
    /// Query whether `.mitos/` config can be loaded
    LocalConfig,
...
pub enum TrustStatus {
    ...
    Untrusted,
    /// Workspace was previously trusted, but the `.mitos/` tree has changed since the grant — the
...
pub enum ImplicitTrustLevel {
    ...
    None,
    /// Mitos-launched server processes (LSP and DAP) start implicitly. Workspace-local config and
...
pub fn build_trusted_globs(patterns: &[String]) -> GlobSet {
    ...
    for pattern in patterns {
        let expanded = mitos_stdx::path::expand(pattern);
...
struct CacheEntry {
    status: TrustStatus,
    /// Whether `.mitos/config.toml` or `.mitos/languages.toml` exists in the workspace at the time
...
impl WorkspaceTrust {
    ...
    /// build, `mitos --health`) where prompting isn't meaningful.
    ...
    /// disk. This catches external mutations to `.mitos/` that happened while mitos was running
    ...
    /// `.mitos/` changed) from *Untrusted* (never trusted)
    ...
    /// Mark `workspace` trusted. Snapshots the current `.mitos/` hash.
...
fn has_local_config(workspace: &Path) -> bool {
    workspace.join(".mitos").join("config.toml").exists()
        || workspace.join(".mitos").join("languages.toml").exists()
}

fn demote_for_query(status: TrustStatus, query: TrustQuery) -> TrustStatus {
    // Stale workspaces have their `.mitos/` config changed since trust was granted. LSP launches
...
// filename derives from the path, so concurrent mitos instances writing trust for *different*
...
/// SHA-256 of all files under `.mitos/`, used to detect changes to local config after trust was
/// granted. Returns `None` if `.mitos/` is absent or has no files, so a workspace with no local
/// config can still be trusted.
pub fn compute_workspace_hash(workspace: &Path) -> Option<String> {
    let mitos_dir = workspace.join(".mitos");
    if !mitos_dir.is_dir() {
        return None;
    }

    let mut files: Vec<PathBuf> = Vec::new();
    walk(&mitos_dir, &mut files);
    if files.is_empty() {
        return None;
    }
    files.sort();
    ...
    let mut hasher = Sha256::new();
    for file in &files {
        let rel = file.strip_prefix(&mitos_dir).unwrap_or(file);
...
fn walk(dir: &Path, out: &mut Vec<PathBuf>) {
    ...
    for entry in entries.flatten() {
        ...
        if direct.is_dir() {
            ...
            // Symlinks to files (extremely common: dotfiles managers symlink `.mitos/config.toml`
...
mod test {
    ...
    #[test]
    fn hash_changes_when_mitos_dir_changes() {
        let dir = tempfile::tempdir().unwrap();
        let workspace = dir.path();

        let empty = compute_workspace_hash(workspace);
        assert_eq!(empty, None, ".mitos/ absent should hash to None");

        write_file(&workspace.join(".mitos").join("config.toml"), "a = 1");
        ...
        write_file(&workspace.join(".mitos").join("config.toml"), "a = 2");
        let h2 = compute_workspace_hash(workspace).expect("has files");
        assert_ne!(h1, h2);

        // Added file → different hash
        write_file(&workspace.join(".mitos").join("languages.toml"), "");
    ...
    fn workspace_restricted_hides_when_no_local_config() {
        ...
        // Untrusted + no .mitos/ → indicator should hide.
        assert!(!trust.workspace_restricted(workspace));

        // Untrusted + .mitos/config.toml exists → indicator should show.
        write_file(&workspace.join(".mitos").join("config.toml"), "a = 1");
    ...
    fn set_config_invalidates_cache_so_stale_is_detected() {
        ...
        // after `:workspace-trust` was run. If the user (or another process) modified `.mitos/`
        // while mitos was running, subsequent queries kept returning Trusted even though
        ...
        // External mutation while mitos is running.
        write_file(&workspace.join(".mitos").join("config.toml"), "a = 2");
    ...
    fn hash_includes_symlinked_config_file() {
        // Regression: `.mitos/config.toml` is commonly a symlink to an external location (dotfiles
        ...
        fs::create_dir_all(workspace.join(".mitos")).unwrap();
        symlink(&external, workspace.join(".mitos").join("config.toml")).unwrap();
    ...
    fn hash_distinguishes_file_split() {
        ...
        let split = dir1.path();
        write_file(&split.join(".mitos").join("foo.toml"), "a");
        write_file(&split.join(".mitos").join("bar.toml"), "b");

        let dir2 = tempfile::tempdir().unwrap();
        let merged = dir2.path();
        write_file(&merged.join(".mitos").join("foo.toml"), "a\0bar.toml\0b");
    ...
    fn workspace_restricted_detects_stale() {
        // Regression: a previously-trusted workspace whose .mitos/ has since been modified must be
        ...
        // Mutate `.mitos/` and bust the in-memory cache to force a re-read.
        write_file(&workspace.join(".mitos").join("config.toml"), "a = 2");
        trust.inner.lock().remove(workspace);
