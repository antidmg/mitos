...
fn ensure_git_is_available() -> Result<()> {
    mitos_stdx::env::which("git")?;
    Ok(())
}

/// Print a notice if the current workspace has a `.mitos/languages.toml` that we *would* have
...
fn warn_if_workspace_languages_skipped(trust: &crate::workspace_trust::WorkspaceTrust) {
    ...
    println!(
        "Note: workspace `{}` was skipped because the workspace is not trusted. Run \
         `:workspace-trust` from an interactive mitos session in this workspace to opt in.",
...
fn get_grammar_configs() -> Result<Vec<GrammarConfiguration>> {
    ...
    // them into `.so` files mitos later loads at runtime. If we let workspace
    // `.mitos/languages.toml` in through `fully_trusted`, a malicious workspace could inject a
...
fn build_grammar(grammar: GrammarConfiguration, target: Option<&str>) -> Result<BuildStatus> {
    ...
    let grammar_dir_entries = grammar_dir.read_dir().with_context(|| {
        format!(
            "Failed to read directory {:?}. Did you use 'mitos --grammar fetch'?",
            grammar_dir
        )
    })?;
    ...
    if grammar_dir_entries.count() == 0 {
        return Err(anyhow!(
            "Directory {:?} is empty. Did you use 'mitos --grammar fetch'?",
            grammar_dir
        ));
    };
