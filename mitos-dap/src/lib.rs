...
#[derive(Debug)]
pub enum Request {
    RunInTerminal(<requests::RunInTerminal as mitos_dap_types::Request>::Arguments),
    StartDebugging(<requests::StartDebugging as mitos_dap_types::Request>::Arguments),
}

impl Request {
    pub fn parse(command: &str, arguments: Option<serde_json::Value>) -> Result<Self> {
        use mitos_dap_types::Request as _;
