...
use mitos_view::graphics::{CursorKind, Rect, UnderlineStyle};
use ratatui::{
    ...
    W: Write,
{
    pub fn new(buffer: W, config: Config) -> CrosstermBackend<W> {
        // mitos is not usable without colors, but crossterm will disable
    ...
    W: Write,
{
    ...
    fn get_theme_mode(&self) -> Option<mitos_view::theme::Mode> {
        None
    }

    fn set_background_color(&mut self, _color: Option<mitos_view::theme::Color>) -> io::Result<()> {
        Ok(())
    }
}
    ...
    W: Write,
{
    ...
    fn set_background_color(&mut self, color: Option<mitos_view::theme::Color>) -> io::Result<()> {
